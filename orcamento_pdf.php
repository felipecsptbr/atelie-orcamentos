...
class PDF extends FPDF {
    ...
    function Header() {
        ...
        $this->Cell(0, 8, utf8_decode($this->config['nome_atelie']), 0, 1);
        ...
        if (!empty($dados_atelie)) {
            $this->Cell(0, 5, utf8_decode(implode(' | ', $dados_atelie)), 0, 1);
        }
    ...
    function DadosBox($titulo, $dados, $w) {
        ...
        $this->Cell($w, 6, utf8_decode($titulo), 1, 1, 'C', true);
        
        $this->SetFont('Arial', '', 9);
        foreach ($dados as $label => $valor) {
            $this->Cell($w/2, 5, utf8_decode($label . ':'), 1, 0);
            $this->Cell($w/2, 5, utf8_decode($valor), 1, 1);
        }
    }
}
...
$pdf->Cell(5, 6, '', 0, 0);
$pdf->Cell(95, 6, utf8_decode('DADOS DO ORÇAMENTO'), 1, 1, 'C', true);

// Cliente
$pdf->SetFont('Arial', '', 9);
$pdf->Cell(30, 5, 'Nome:', 1, 0);
$pdf->Cell(65, 5, utf8_decode($orcamento['cliente_nome']), 1, 0);
$pdf->Cell(5, 5, '', 0, 0);
$pdf->Cell(30, 5, 'Data:', 1, 0);
$pdf->Cell(65, 5, date('d/m/Y', strtotime($orcamento['data_orcamento'])), 1, 1);

$pdf->Cell(30, 5, 'Telefone:', 1, 0);
$pdf->Cell(65, 5, utf8_decode($orcamento['cliente_telefone']), 1, 0);
$pdf->Cell(5, 5, '', 0, 0);
...
if ($orcamento['cliente_email']) {
    ...
    $pdf->Cell(65, 5, utf8_decode($orcamento['cliente_email']), 1, 0);
} else {
    $pdf->Cell(95, 5, '', 1, 0);
}
$pdf->Cell(5, 5, '', 0, 0);
$pdf->Cell(30, 5, 'Status:', 1, 0);
$pdf->Cell(65, 5, utf8_decode(ucfirst(str_replace('_', ' ', $orcamento['status']))), 1, 1);
...
$pdf->Cell(85, 7, utf8_decode('Descrição'), 1, 0, 'C', true);
...
foreach ($itens as $item) {
    ...
    $height = 6;
    $nb_lines = $pdf->NbLines(85, utf8_decode($descricao));
    if ($nb_lines > 1) {
        $height = 5 * $nb_lines;
    }
    ...
    $pdf->MultiCell(85, 5, utf8_decode($descricao), 1);
    $pdf->SetXY($x + 95, $y);
...
if ($orcamento['desconto_valor'] > 0) {
    ...
    $pdf->Cell(150, 6, utf8_decode($desconto_label . ':'), 1, 0, 'R');
...
$pdf->SetFont('Arial', 'B', 10);
$pdf->Cell(0, 6, utf8_decode('INFORMAÇÕES ADICIONAIS'), 0, 1);
$pdf->SetFont('Arial', '', 9);

if ($orcamento['prazo_execucao']) {
    $pdf->Cell(50, 5, utf8_decode('Prazo de execução:'), 0, 0);
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->Cell(0, 5, utf8_decode($orcamento['prazo_execucao']), 0, 1);
    $pdf->SetFont('Arial', '', 9);
}
...
if ($orcamento['forma_pagamento']) {
    ...
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->Cell(0, 5, utf8_decode($orcamento['forma_pagamento']), 0, 1);
    $pdf->SetFont('Arial', '', 9);
}
...
if ($orcamento['observacoes']) {
    $pdf->Ln(2);
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->Cell(0, 5, utf8_decode('Observações:'), 0, 1);
    $pdf->SetFont('Arial', '', 9);
    $pdf->MultiCell(0, 5, utf8_decode($orcamento['observacoes']));
}
